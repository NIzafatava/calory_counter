...
def HomePageView(request):
    ...
    breakfast_all_food_today = PostFood.objects.filter(
        profile=calories, meat_type__contains="breakfast"
    )
    brunch_all_food_today = PostFood.objects.filter(
        profile=calories, meat_type__contains="brunch"
    )
    lunch_all_food_today = PostFood.objects.filter(
        profile=calories, meat_type__contains="lunch"
    )
    dinner_all_food_today = PostFood.objects.filter(
        profile=calories, meat_type__contains="dinner"
    )
    supper_all_food_today = PostFood.objects.filter(
        profile=calories, meat_type__contains="supper"
    )
    snacks_all_food_today = PostFood.objects.filter(
        profile=calories, meat_type__contains="snacks"
    )
    calorie_goal_status = (
        calorie_goal - calories.total_calorie + calories.total_calorie_exercise
    )
